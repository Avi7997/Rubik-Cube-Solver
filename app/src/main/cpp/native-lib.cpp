                ...
                if( applicableMoves[phase] & (1 << move) ){
                    ...
                    if( newDir  &&  newDir != oldDir ){
                        ...
                        for( int i=0; i<(int)algorithm.size(); i++ ){
                            ...
                            string temp = "UDFBLR";
                            temp = temp[algorithm[i]/3];
                            //int te = algorithm[i]%3+1;
                            string te = "0123";
                            te = te[algorithm[i]%3+1];
                            answer.append(temp);
                            answer.append(te);
                            answer.append(" ");
...
extern "C" JNIEXPORT jstring JNICALL Java_com_io_sefu_rsolverfinalui_BackgroundTask_stringFromJNI(
        JNIEnv *env,
        jobject *//* this *//*,
        jstring string) {
    std::string hello = jstring2string(env,string);
    return env->NewStringUTF(jay(hello).c_str());
}*/
